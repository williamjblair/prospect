...
Start here: the one-page [judge handout](JUDGE_HANDOUT.md), the [reliability benchmark](RELIABILITY_BENCHMARK.md)
(the headline result and its statistics), the [demo script](DEMO.md), and the [Devpost writeup](DEVPOST.md).
The rest of `docs/` is supporting evidence, each generated by a named `./prospect` command.
...
- Overclaiming benchmark: 47.9% of confident AI major-regulator claims contradicted by the measured data
  (46 of 96 comparable claims, 95% CI 38 to 58 percent), 63.9% on famous checkpoints and cytokines. Models
  overclaim famous genes far above the 7.0% rate on data-confirmed non-regulators (permutation p 0.0001),
...
4. Read the reliability benchmark on the overclaim band: about half of confident claims contradicted
   (46 of 96, 95% CI 38 to 58 percent), 63.9% on famous genes at permutation p 0.0001, and a
