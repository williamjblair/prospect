...
def test_campaign_triage_turns_more_aggressive_probe_rows_into_assay_gates():
    ...
    assert triage["summary"]["more_aggressive"] == 11
    assert triage["summary"]["secondary_assay_queue"] == 5
    assert triage["summary"]["capacity_assay_queue"] == 6
    ...
    assert by_gene["SCO2"]["triage_decision"] == "capacity_assay_queue"
...
def test_campaign_triage_writes_json_csv_and_markdown(tmp_path):
    ...
    assert json.loads(out_json.read_text())["summary"]["more_aggressive"] == 11
