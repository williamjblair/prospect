"""Receipt schema v1 documentation and emitter conformance."""
import json
import subprocess
import sys
...
SCHEMA = ROOT / "receipt" / "receipt_schema_v1.json"
...
def test_receipt_schema_v1_is_written_and_names_the_boundary():
    schema = _schema()
    doc = DOC.read_text()

    assert schema["$id"] == "prospect.receipt.v1"
    assert schema["title"] == "Prospect Receipt v1"
    for field in [
        "claim",
        "evidence",
        "artifacts",
...
if __name__ == "__main__":
    test_receipt_schema_v1_is_written_and_names_the_boundary()
    import tempfile
    test_receipt_emitter_outputs_schema_v1_receipts_without_state_mutation(Path(tempfile.mkdtemp()))
