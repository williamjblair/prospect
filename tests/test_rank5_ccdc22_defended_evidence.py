...
def test_ccdc22_packet_records_current_support():
    ...
    assert gates["expanded_external_freeze"]["reason"] == (
        "the expanded hackathon bar still needs new frozen GWAS Catalog, DepMap, conservation, or additional primary-T-cell comparator evidence"
    )
...
def test_ccdc22_packet_survives_current_kills_but_does_not_accept():
    ...
    assert packet["defended_discovery_status"] == "needs_external_freeze"
...
def test_ccdc22_packet_writes_clean_json_and_markdown(tmp_path):
    ...
    assert "needs external freeze" in doc
