"""Build the rank-2 RCC1L defended-evidence packet from frozen packets."""
...
OUT_JSON = DATA / "rcc1l_defended_evidence.json"
OUT_DOC = ROOT / "docs" / "RCC1L_DEFENDED_EVIDENCE.md"
    ...
    return [
        {
            ...
            "reason": (
                "RCC1L has no supporting hit in Shifrut 2018 and Schmidt remains an orthogonal "
        ...
        },
        {
            ...
            "reason": "STRING context does not yet specify a direct stimulated CD4+ activation mechanism",
        },
    ]
...
def _kill_attempts() -> list[dict[str, str]]:
    return [
        ...
        },
        {
            ...
            "result": "survives_current_frozen_evidence",
            "basis": "Rest DE is 58 and K562 DE is 5 in the frozen packets",
        },
        {
...
def build_rcc1l_defended_evidence() -> dict[str, Any]:
    prereg = _load(PREREG_JSON)
    discovery = _load(DISCOVERY_JSON)
    cross_validation = _load(CROSS_VALIDATION_JSON)
    _load(DECISIONS_JSON)
    discovery_row = _row(discovery, "RCC1L")
    cross_row = _row(cross_validation, "RCC1L")
    packet = {
        "phase": "rank_2_rcc1l_defended_evidence",
        "title": "RCC1L defended evidence",
        "gene": "RCC1L",
        ...
        "reproduce_command": "./prospect rcc1l-defended-evidence",
        "next_step": "record RCC1L decision and build rank-3 MCAT defended evidence",
    }
    packet["packet_id"] = _hash_obj("rcc1l_defended", packet)
    return packet
...
def write_rcc1l_defended_evidence(
...
def main() -> None:
    packet = write_rcc1l_defended_evidence()
