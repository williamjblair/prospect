"""Build the rank-3 MCAT defended-evidence packet from frozen packets."""
...
OUT_JSON = DATA / "mcat_defended_evidence.json"
OUT_DOC = ROOT / "docs" / "MCAT_DEFENDED_EVIDENCE.md"
...
def _scored_evidence(discovery_row: dict[str, Any], cross_row: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "status": "evidence_attached",
            "summary": "top partners: " + ", ".join(cross_row["string_network"]["top_partners"][:5]),
        },
        {
    ...
    return [
        {
            ...
            "reason": (
                "MCAT has no supporting hit in Shifrut 2018 and Schmidt remains an orthogonal "
                "cytokine-production phenotype"
            ),
        },
        {
            "rung": "real_world_hook",
            "reason": "the bounded Open Targets overlay has no selected immune or hematologic context for MCAT",
        },
        {
            "rung": "specific_mechanism",
            "reason": (
                "STRING context points to fatty-acid and mitochondrial metabolism, but not a specific "
...
def _kill_attempts() -> list[dict[str, str]]:
    return [
        ...
        },
        {
            ...
            "result": "survives_current_frozen_evidence",
            "basis": "Rest DE is 113 and K562 DE is 20, within the pre-registered ceiling",
        },
        {
        ...
        },
        {
            ...
            "result": "not_cleared",
            "basis": "current STRING context does not establish a specific activation mechanism",
        },
    ]


def build_mcat_defended_evidence() -> dict[str, Any]:
    prereg = _load(PREREG_JSON)
    discovery = _load(DISCOVERY_JSON)
    cross_validation = _load(CROSS_VALIDATION_JSON)
    _load(DECISIONS_JSON)
    discovery_row = _row(discovery, "MCAT")
    cross_row = _row(cross_validation, "MCAT")
    packet = {
        "phase": "rank_3_mcat_defended_evidence",
        "title": "MCAT defended evidence",
        "gene": "MCAT",
        ...
        "reproduce_command": "./prospect mcat-defended-evidence",
        "next_step": "record MCAT decision and build rank-4 RWDD2B defended evidence",
    }
    packet["packet_id"] = _hash_obj("mcat_defended", packet)
    return packet
...
def write_mcat_defended_evidence(
...
def main() -> None:
    packet = write_mcat_defended_evidence()
